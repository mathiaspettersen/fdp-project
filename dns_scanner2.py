...
# Libraries below are from dnspython
...
with open("C:/Users/mathi/OneDrive/Noroff/Noroff/Year 3/FDP/pythonProject_current/Scandinavia/scandinavia_high_ed.txt",
          "r") \
        as domain_name_file:
    ...
    with open('scan1.csv', mode='w', newline='') as file:
        ...
        for line in domain_name_file:
            ...
            dns_answer = dns.resolver.resolve(contents, "SOA", raise_on_no_answer=False)

            print(contents + "    " + str(dns_answer.rrset.ttl))
            ...
            dns_answer = dns.resolver.resolve(contents, "NS", raise_on_no_answer=False)
            for answer in dns_answer:
            ...
            dns_answer = dns.resolver.resolve(contents, "A", raise_on_no_answer=False)
            for answer in dns_answer:
                print("A -------------------------------------------")
                print(answer)  # IPv4 address
                print("A ---------\n")

            dns_answer = dns.resolver.resolve(contents, "AAAA", raise_on_no_answer=False)
            for answer in dns_answer:
                print("AAAA -------------------------------------------")
            ...
            dns_answer = dns.resolver.resolve(contents, "DNSKEY", raise_on_no_answer=False)
            for answer in dns_answer:
                # new_ans = answer
                print("DNSKEY -------------------------------------------")
            ...
            if dns_answer.rrset is not None:
                print("RRSET -----------------------------------------------")
                print(dns_answer.rrset.ttl)  # Resource Record set
                print("RRSET -------------\n")

            if dns_answer.rrset is not None:
                print("RRSET NAME -----------------------------------------------")
                print(dns_answer.rrset.rdtype)  # Resource Record name
                print("RRSET NAME -------------\n")
            ...
            def get_dnssec(dns_resolver_dnssec, domain_name2):
                ...
                ns_address = response.rrset[0].to_text()  # IPv4
                ...
                # send the query
                response = dns.query.udp(request, ns_address, timeout=1.0)
                if response.rcode() != 0:
                    raise Exception("get_dnssec_status: response code was not 0")
                # the dnssec_answer should contain both DNSKEY and RRSIG(DNSKEY)

                dnssec_answer = response.answer
                # if len(dnssec_answer) != 2:
                #    # an exception was raised
                #    raise Exception("get_dnssec_status: length of dnssec_answer != 2, " +
                #                    str(len(dnssec_answer)))
                ...
                if dnssec_answer:
                    ...
                    substring = "[<25"
                    # substring1 = "6" or "7"
                    algo = str(dnssec_answer).split(substring, 1)[1].strip()
                ...
                try:
                    ...
                    dns.dnssec.validate(dnssec_answer[0], dnssec_answer[1], {name: dnssec_answer[0]})
